...
def infer_mask_helper(
    ...
    # Convert mask to numpy and ensure it's in the correct format for OpenCV
    ...
    # convert to tensor for show_cam_on_image
    #mask_t = torch.from_numpy(mask_np).to(cfg.device)
    mask_t = mask.squeeze(0).to(cfg.device)
    model.show_cam_on_image(mask_t, output_path)
    #print(f"Saved refined mask to {output_path}")

    return mask_t, difference
...
class iSeg(iSeg_base):
    ...
    def infer_refined_mask(self, latent, prompt):
        ...
        final_attention_map = F.interpolate(final_attention_map[None], size=(512,512),#(image.shape[-2:]),
